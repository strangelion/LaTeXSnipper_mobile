...
function _showToast(text) {
  ...
  el.innerHTML = `<p>${text}</p><button class="toast-close">${t('common.ok')}</button>`;
  ...
  overlay.innerHTML = `
    <div class="update-dialog">
      <div class="update-dialog-header">
        <h3>${t('update.available').replace('{{version}}', version)}</h3>
        <button class="update-dialog-close">&times;</button>
      </div>
      <div class="update-dialog-body">${releaseNotes}</div>
      <div class="update-dialog-footer">
        <button class="ocr-btn secondary" id="updateCancel">${t('update.later')}</button>
        <a href="${url}" target="_blank" rel="noopener" class="ocr-btn" id="updateGo" style="text-align:center;text-decoration:none;">${t('update.download')}</a>
      </div>
    </div>`;
...
export async function checkForUpdateNow() {
  try {
    ...
    if (!info) {
      _showToast(t('check.failed'));
      return { found: false, error: true };
    }
    if (!_isNewer(info.version, _currentVersion)) {
      _showToast(t('update.upToDate'));
    ...
    _showToast(t('check.failed') + ': ' + (e.message || ''));
    return { found: false, error: true };
  }
}
